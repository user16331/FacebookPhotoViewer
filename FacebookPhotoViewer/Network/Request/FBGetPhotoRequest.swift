...
/// Get Facebook full size photo Request
internal class FBGetPhotoRequest: FBRequest {
    ...
    internal func execute(_ callback: @escaping (UIImage?, FBRequestError?) -> Void) {
        ...
        guard let url = URL(string: "https://graph.facebook.com/\(self.photoId)/picture?return_ssl_resources=1&access_token=\(tokenString)") else {
